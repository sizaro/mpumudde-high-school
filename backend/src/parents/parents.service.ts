...
export class ParentsService {
  ...
  async create(createParentDto: CreateParentDto, uploadedByUserId?: string) {
    ...
    if (
      new Set(studentLinks.map((link) => link.studentId)).size !==
      studentLinks.length
    ) {
      throw new BadRequestException(
    ...
    const loginEmail = shouldCreateLogin
      ? (createParentDto.loginEmail?.trim().toLowerCase() ??
        (await this.generateUniqueLoginEmail(
    ...
    const temporaryPassword = shouldCreateLogin
    ...
    const createdId = await this.prisma.$transaction(async (tx) => {
      ...
      const documentCategoryIds = (createParentDto.documents ?? []).map(
        (document) => document.documentCategoryId,
      );
      if (documentCategoryIds.length) {
        const categoryCount = await tx.documentCategory.count({
          where: {
            id: { in: documentCategoryIds },
            entityType: "PARENT",
            isActive: true,
          },
        });
        if (categoryCount !== new Set(documentCategoryIds).size) {
          throw new BadRequestException(
            "One or more guardian document categories are invalid.",
          );
        }
      }
    ...
    return {
      ...
      credentials: temporaryPassword
        ? {
            email: loginEmail!,
            username: createParentDto.username || loginEmail!,
          }
        : undefined,
    };
  }
    ...
    return this.prisma.parent.findMany({
      ...
      include: {
        ...
        students: {
          ...
          include: {
            student: {
              include: { schoolClass: true, academicYear: true, term: true },
            },
          },
        },
      },
    ...
    const response: Record<string, unknown> = {
      ...
      familySummary: children.reduce(
        (summary, child) => {
          ...
          summary.childrenWithBalances +=
            finance.outstandingBalance > 0 ? 1 : 0;
          summary.fullyPaidChildren +=
            finance.totalExpected > 0 && finance.outstandingBalance <= 0
              ? 1
              : 0;
        ...
        },
        {
          ...
          childrenWithBalances: 0,
          fullyPaidChildren: 0,
          recentAttendanceRecords: 0,
          recentPresent: 0,
          recentAbsent: 0,
          recentLate: 0,
        },
  ...
  async getChildAttendance(
      ...
      filters.startDate || filters.endDate
        ? {
            ...(filters.startDate
              ...
              : {}),
            ...(filters.endDate
              ? { lte: new Date(`${filters.endDate}T23:59:59+03:00`) }
              : {}),
          }
        : undefined;
    ...
    return {
      ...
      subjects: Array.from(
        new Map(
          records.map((record) => [
            record.attendanceSession.subject.id,
            record.attendanceSession.subject,
          ]),
        ).values(),
      ),
      records: records.map((record) => ({
        ...
        subjectId: record.attendanceSession.subject.id,
  ...
  private buildStudentDashboard(student: any) {
    ...
    const charges = Array.isArray(student.financeCharges)
    ...
    const totalPaid = charges.length
      ? charges.reduce(
          (sum: number, charge: any) => sum + (charge.paidAmount ?? 0),
          0,
        )
    ...
    const financeSummary = {
      payments: payments.map((payment: any) => ({
        ...
        accountStatus: payment.studentCharge
          ? this.chargeStatusFromTotals(
              payment.studentCharge.expectedAmount ?? 0,
              payment.studentCharge.paidAmount ?? 0,
              payment.studentCharge.waivedAmount ?? 0,
            )
          : null,
        chargeId: payment.studentCharge?.id ?? null,
        chargeExpectedAmount: payment.studentCharge?.expectedAmount ?? null,
        chargePaidAmount: payment.studentCharge?.paidAmount ?? null,
        chargeWaivedAmount: payment.studentCharge?.waivedAmount ?? null,
        chargeBalance: payment.studentCharge
          ? Math.max(
              0,
              (payment.studentCharge.expectedAmount ?? 0) -
                (payment.studentCharge.paidAmount ?? 0) -
          ...
          : null,
        academicYear:
          payment.studentCharge?.financeStructure?.academicYear?.name ?? null,
        term: payment.studentCharge?.financeStructure?.term?.name ?? null,
      })),
      charges: charges.map((charge: any) => ({
        ...
        balance: Math.max(
          0,
          (charge.expectedAmount ?? 0) -
            (charge.paidAmount ?? 0) -
            (charge.waivedAmount ?? 0),
        ),
        status: this.chargeStatusFromTotals(
          charge.expectedAmount ?? 0,
          charge.paidAmount ?? 0,
          charge.waivedAmount ?? 0,
        ),
        paymentProofs: (charge.payments ?? [])
          .filter((payment: any) => Boolean(payment.proofUrl))
          .map((payment: any) => ({
            id: payment.id,
            amount: payment.amount ?? 0,
            method: payment.method ?? null,
    ...
    return {
      ...
      academicPerformance: {
        message:
          "Academic performance details are not available in the current schema.",
        grades: Array.isArray(student.grades) ? student.grades : [],
      },
    };
  }

  private chargeStatusFromTotals(
  ...
  async updateComplete(
    ...
    if (links.length === 0)
      throw new BadRequestException(
        "A guardian must remain linked to at least one student.",
      );
    ...
    await this.prisma.$transaction(async (tx) => {
      const students = await tx.student.findMany({
        where: { id: { in: links.map((link) => link.studentId) } },
        select: { id: true },
      });
      if (students.length !== links.length)
        throw new BadRequestException(
          "One or more selected students do not exist.",
        );
      const existing = await tx.studentParent.findMany({
        where: { parentId: id, isActive: true },
      });
      const desiredIds = new Set(links.map((link) => link.studentId));
      for (const removed of existing.filter(
        (link) => !desiredIds.has(link.studentId),
      )) {
        await tx.studentParent.update({
          ...
          data: {
            ...
            isPrimary: false,
            unlinkedAt: new Date(),
            unlinkedByUserId: changedByUserId,
            unlinkReason: "Removed during guardian update",
          },
        });
      }
      for (const link of links) {
        ...
        await tx.studentParent.upsert({
          where: {
            studentId_parentId: { studentId: link.studentId, parentId: id },
          },
          create: {
            studentId: link.studentId,
            parentId: id,
  ...
  private async updateParentRecord(
    ...
    if (updateParentDto.firstName !== undefined)
      data.firstName = updateParentDto.firstName;
    if (updateParentDto.lastName !== undefined)
      data.lastName = updateParentDto.lastName;
    if (updateParentDto.gender !== undefined)
      data.gender = updateParentDto.gender;
    if (updateParentDto.phone !== undefined) data.phone = updateParentDto.phone;
    if (updateParentDto.address !== undefined)
      data.address = updateParentDto.address;
    if (updateParentDto.occupation !== undefined)
      data.occupation = updateParentDto.occupation;
    if (updateParentDto.profilePhoto !== undefined)
      data.profilePhoto = updateParentDto.profilePhoto;
    if (updateParentDto.relationship !== undefined)
      data.relationship = updateParentDto.relationship;
    if (updateParentDto.identityDocumentType !== undefined)
      data.identityDocumentType = updateParentDto.identityDocumentType;
    if (updateParentDto.identityDocumentUrl !== undefined)
      data.identityDocumentUrl = updateParentDto.identityDocumentUrl;
  ...
  async resetPortalPassword(parentId: string, requestedEmail?: string) {
    const parent = await this.prisma.parent.findUnique({
      where: { id: parentId },
      include: { user: true },
    });
    ...
    if (!parent.userId || !parent.user)
      throw new BadRequestException(
        "This guardian does not have a portal account.",
      );

    const nextEmail = requestedEmail?.trim().toLowerCase();
    if (nextEmail && nextEmail !== parent.user.email) {
  ...
  async unlinkStudent(
    ...
    if (!relation || !relation.isActive)
      throw new NotFoundException(
        "This child is not actively linked to the guardian.",
      );
    ...
    if (activeRelationships <= 1) {
      throw new BadRequestException(
        "Link another student before removing the guardian’s final active relationship, or archive the guardian.",
      );
    }
  ...
  async addDocument(
    ...
    if (!parent) throw new NotFoundException("Parent not found.");
    if (!category)
      throw new BadRequestException("Invalid guardian document category.");
    ...
    return this.prisma.$transaction(async (tx) => {
      await tx.studentParent.updateMany({
        ...
        data: {
          ...
          isPrimary: false,
          unlinkedAt: new Date(),
          unlinkedByUserId: archivedByUserId,
          unlinkReason: "Guardian archived",
        },
      });
      ...
      if (parent.userId) {
        await tx.user.update({
          where: { id: parent.userId },
          data: { isActive: false, isLoggedIn: false },
        });
      }
      return archived;
    });
