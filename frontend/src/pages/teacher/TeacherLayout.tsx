...
export default function TeacherLayout() {
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex w-64 flex-col gap-1 overflow-y-auto bg-gray-900 px-3 py-6 text-white transition-transform lg:static lg:w-56 lg:translate-x-0 ${mobileNavOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        <button
          type="button"
          onClick={() => setMobileNavOpen(false)}
        ...
        {links.map((link) => (
          <NavLink
            ...
            onClick={() => setMobileNavOpen(false)}
            className={({ isActive }) =>
              `rounded px-3 py-2 text-sm ${isActive ? "bg-blue-600 text-white" : "text-gray-300 hover:bg-gray-700"}`
            }
          >
            {link.label}
          </NavLink>
