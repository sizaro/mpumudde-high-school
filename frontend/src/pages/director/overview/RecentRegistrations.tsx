...
export default function RecentRegistrations({
  ...
  return (
    <section className="overflow-y-auto max-w-full rounded-3xl border border-slate-200 bg-white shadow-sm">
      <div className="border-b border-slate-100 p-6">
        <h2 className="text-lg font-semibold text-slate-900">
          Recent registrations
        </h2>
        <p className="mt-1 text-sm text-slate-500">
          The newest student and teacher records.
        </p>
      </div>
          ...
          <div className="space-y-1">
            {data.recentStudents.length ? (
              data.recentStudents.map((student) => (
                <Link
                  key={student.id}
                  to={`students/profile?id=${student.id}`}
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="block truncate text-xs text-slate-500">
                      ...
                      {student.schoolClass?.name ?? "Class not assigned"} ·{" "}
                      {dateLabel(student.createdAt)}
                    </span>
                  </span>
                  <ArrowRight
                    size={15}
              ...
              data.recentTeachers.map((teacher) => (
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="block truncate text-xs text-slate-500">
                      {teacher.employment?.employeeNumber ??
                        "No employee number"}{" "}
                      · {teacher.employment?.position ?? "Position not set"} ·{" "}
                      {dateLabel(teacher.createdAt)}
                    </span>
                  </span>
                  <ArrowRight
                    size={15}
