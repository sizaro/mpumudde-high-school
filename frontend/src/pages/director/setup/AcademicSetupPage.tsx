...
export default function AcademicSetupPage() {
  ...
  const prepareTerms = async () => {
    await Promise.all(
      academicYears.map((year) => SetupService.ensureStandardTerms(year.id)),
    );
    await loadData();
  };

  const selectedYearNames = useMemo(
    () => academicYears.map((item) => item.name),
    [academicYears],
  );

  const handleCreateAcademicYear = async () => {
    if (!form.academicYearName.trim()) return;
    await SetupService.createAcademicYear({
    ...
    if (editingTermId) {
      ...
      if (termToUpdate) {
        await SetupService.updateTerm(editingTermId, {
          ...
          feeAmount: form.termFeeAmount
            ? Number(form.termFeeAmount)
            : termToUpdate.feeAmount,
  ...
  const handleCreateSubject = async () => {
    ...
    await SetupService.createSubject({
      ...
      code: form.subjectCode.trim() || undefined,
    });
  ...
  const handleCreateStudentCategory = async () => {
    ...
    await SetupService.createStudentCategory({
      name: form.studentCategoryName.trim(),
    });
  ...
  const handleCreateFinanceStructure = async () => {
    if (
      !form.financeAcademicYearId ||
      !form.financeTermId ||
      !form.financeClassId ||
      !form.financeStudentCategoryId ||
      !form.financeFeeTypeId ||
      !form.financeExpectedAmount
    )
      return;
  ...
  const toggleAcademicYear = async (year: AcademicYear) => {
    await SetupService.updateAcademicYear(year.id, {
      isActive: !year.isActive,
    });
    setMessage(`Academic year ${year.isActive ? "deactivated" : "activated"}.`);
    await loadData();
  };

  const toggleClass = async (schoolClass: SchoolClass) => {
    await SetupService.updateClass(schoolClass.id, {
      isActive: !schoolClass.isActive,
    });
    setMessage(`Class ${schoolClass.isActive ? "deactivated" : "activated"}.`);
    await loadData();
  };

  const toggleSubject = async (subject: Subject) => {
    await SetupService.updateSubject(subject.id, {
      isActive: !subject.isActive,
    });
    setMessage(`Subject ${subject.isActive ? "deactivated" : "activated"}.`);
    await loadData();
  };

  const toggleStudentCategory = async (category: StudentCategory) => {
    await SetupService.updateStudentCategory(category.id, {
      isActive: !category.isActive,
    });
    setMessage(
      `Student category ${category.isActive ? "deactivated" : "activated"}.`,
    );
    await loadData();
  };
      ...
      <div className="mb-5 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-500">
            Configure academic years, terms, classes, subjects, and student
            categories. Fee types and fee structures are managed in Finance.
          </p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-2 text-sm text-slate-600">
          {selectedYearNames.length > 0
            ? `${selectedYearNames.join(", ")}`
      ...
      {!loading && activeTab === "academicYears" ? (
        <div className="grid gap-4 lg:grid-cols-[1.1fr_0.85fr]">
          <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <p className="mt-2 text-sm text-slate-500">
              Add and activate academic years such as 2026.
            </p>
              ...
              {academicYears.map((year) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    <span
                      className={`rounded-full px-3 py-1 text-xs font-semibold ${year.isActive ? "bg-emerald-100 text-emerald-700" : "bg-slate-200 text-slate-600"}`}
                    >
                      {year.isActive ? "Active" : "Inactive"}
                    </span>
                    <button
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <label className="mt-4 block text-sm font-medium text-slate-700">
              ...
              <input
                ...
                onChange={(event) =>
                  setForm((current) => ({
                    ...current,
                    academicYearName: event.target.value,
                  }))
                }
            ...
            <button
              ...
              className="mt-4 rounded-2xl bg-slate-900 px-5 py-3 text-sm font-medium text-white"
            >
              Save Academic Year
            </button>
          </div>
      ...
      {!loading && activeTab === "terms" ? (
        <div className="grid gap-4 lg:grid-cols-[1.1fr_0.85fr]">
          <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <p className="mt-2 text-sm text-slate-500">
              Each academic year always has Term 1, Term 2, and Term 3. Turn a
              term on or off when needed.
            </p>
            <div className="mt-4 space-y-3">
                    ...
                    <div>
                      ...
                      {terms.find((t) => t.name === termName)?.startDate && (
                        <p className="mt-1 text-xs text-slate-500">
                          ...
                          -{" "}
                          {new Date(
                            terms.find((t) => t.name === termName)?.endDate ||
                              "",
                          ).toLocaleDateString()}
                        </p>
                      )}
                      ...
                      onClick={() => {
                        ...
                        if (term) {
                          ...
                          setForm((current) => ({
                            ...
                            termFeeAmount: term.feeAmount
                              ? String(term.feeAmount)
                              : "",
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            <h2 className="text-xl font-semibold text-slate-900">
              {editingTermId ? "Term settings" : "Set up standard terms"}
            </h2>
            {editingTermId ? (
              <>
                <label className="mt-4 block text-sm font-medium text-slate-700">
                ...
                <label className="mt-4 block text-sm font-medium text-slate-700">
                  ...
                  <input
                    type="number"
                    value={form.termFeeAmount}
                    onChange={(event) =>
                      setForm((current) => ({
                        ...current,
                        termFeeAmount: event.target.value,
                      }))
                    }
                    className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3"
                    placeholder="0"
                ...
                <label className="mt-4 flex items-center gap-2 text-sm font-medium text-slate-700">
                  <input
                    type="checkbox"
                    checked={form.termIsActive}
                    onChange={(event) =>
                      setForm((current) => ({
                        ...current,
                        termIsActive: event.target.checked,
                      }))
                    }
                  />
                  Active term
                ...
                <div className="mt-4 flex gap-2">
                  <button
                    ...
                    className="flex-1 rounded-2xl bg-slate-900 px-5 py-3 text-sm font-medium text-white"
                  >
                    Save Changes
                  </button>
                  <button
                    ...
                    className="flex-1 rounded-2xl border border-slate-200 px-5 py-3 text-sm font-medium text-slate-700"
                  >
                    Cancel
                  </button>
                </div>
              ...
              <>
                <p className="mt-4 text-sm text-slate-500">
                  Create any missing standard terms for the academic years
                  already in the system.
                </p>
                <button
                  type="button"
                  onClick={() => void prepareTerms()}
                  className="mt-4 rounded-2xl bg-slate-900 px-5 py-3 text-sm font-medium text-white"
                >
                  Create missing Term 1, 2 and 3
                </button>
              </>
            )}
          </div>
              ...
              {classes.length ? (
                classes.map((item) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      <span
                        className={`rounded-full px-3 py-1 text-xs font-semibold ${item.isActive ? "bg-emerald-100 text-emerald-700" : "bg-slate-200 text-slate-600"}`}
                      >
      ...
      {!loading && activeTab === "subjects" ? (
        <div className="grid gap-4 lg:grid-cols-[1.1fr_0.85fr]">
          <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <p className="mt-2 text-sm text-slate-500">
              Create the school subject catalogue once. Teachers are assigned
              subjects, then may choose any active class when taking attendance.
            </p>
            <div className="mt-4 space-y-3">
              {subjects.length ? (
                subjects.map((item) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        type="button"
                        onClick={() => void toggleSubject(item)}
                        className="rounded-xl border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-700"
                      >
                        {item.isActive ? "Deactivate" : "Activate"}
                      </button>
        ...
        <div className="grid gap-4 lg:grid-cols-[1.1fr_0.85fr]">
                ...
                studentCategories.map((item) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        type="button"
                        onClick={() => void toggleStudentCategory(item)}
                        className="rounded-xl border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-700"
                      >
                        {item.isActive ? "Deactivate" : "Activate"}
                      </button>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <label className="mt-4 block text-sm font-medium text-slate-700">
              Category Name
              <select
                value={form.studentCategoryName}
                onChange={(event) =>
                  setForm((current) => ({
                    ...current,
                    studentCategoryName: event.target.value,
                  }))
                }
                className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3"
              >
            ...
            <button
              type="button"
              onClick={handleCreateStudentCategory}
              className="mt-4 rounded-2xl bg-slate-900 px-5 py-3 text-sm font-medium text-white"
            >
              Save Category
            </button>
            ...
            <label className="mt-4 block text-sm font-medium text-slate-700">
              ...
              <input
                ...
                onChange={(event) =>
                  setForm((current) => ({
                    ...current,
                    feeTypeName: event.target.value,
                  }))
                }
                className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3"
                placeholder="e.g. Registration, Tuition, Reams, Development"
              />
            </label>
            <button
              type="button"
      ...
      {!loading && activeTab === "financeStructures" ? (
        <div className="grid gap-4 lg:grid-cols-[1.1fr_0.85fr]">
          <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 space-y-3">
              {financeStructures.length ? (
                financeStructures.map((item) => (
                  <div
                    key={item.id}
                    className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3"
                  >
                    <p className="font-medium text-slate-900">
                      {item.academicYear?.name} • {item.term?.name} •{" "}
                      {item.schoolClass?.name} • {item.studentCategory?.name}
                    </p>
                    <p className="mt-1 text-sm text-slate-500">
                      {item.feeType?.name}:{" "}
                      {item.expectedAmount.toLocaleString()} UGX
                    </p>
                  </div>
                ))
              ) : (
                <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-4 py-4 text-sm text-slate-500">
                  No finance structures created yet.
                </div>
              )}
            </div>
          </div>
          <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <label className="mt-4 block text-sm font-medium text-slate-700">
              Academic Year
              <select
                value={form.financeAcademicYearId}
                onChange={(event) =>
                  setForm((current) => ({
                    ...current,
                    financeAcademicYearId: event.target.value,
                  }))
                }
                className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3"
              >
            ...
            <label className="mt-4 block text-sm font-medium text-slate-700">
              Term
              <select
                value={form.financeTermId}
                disabled={!form.financeAcademicYearId}
                onChange={(event) =>
                  setForm((current) => ({
                    ...current,
                    financeTermId: event.target.value,
                  }))
                }
              ...
              >
                ...
                {terms
                  .filter(
                    (term) =>
                      term.academicYearId === form.financeAcademicYearId,
                  )
                  .map((item) => (
            ...
            <label className="mt-4 block text-sm font-medium text-slate-700">
              Class
              <select
                value={form.financeClassId}
                onChange={(event) =>
                  setForm((current) => ({
                    ...current,
                    financeClassId: event.target.value,
                  }))
                }
                className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3"
              >
            ...
            <label className="mt-4 block text-sm font-medium text-slate-700">
              Student Category
              <select
                value={form.financeStudentCategoryId}
                onChange={(event) =>
                  setForm((current) => ({
                    ...current,
                    financeStudentCategoryId: event.target.value,
                  }))
                }
                className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3"
              >
            ...
            <label className="mt-4 block text-sm font-medium text-slate-700">
              Fee Type
              <select
                value={form.financeFeeTypeId}
                onChange={(event) =>
                  setForm((current) => ({
                    ...current,
                    financeFeeTypeId: event.target.value,
                  }))
                }
                className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3"
              >
            ...
            <label className="mt-4 block text-sm font-medium text-slate-700">
              ...
              <input
                type="number"
                value={form.financeExpectedAmount}
                onChange={(event) =>
                  setForm((current) => ({
                    ...current,
                    financeExpectedAmount: event.target.value,
                  }))
                }
                className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3"
                placeholder="500000"
            ...
            <button
              type="button"
              onClick={handleCreateFinanceStructure}
              className="mt-4 rounded-2xl bg-slate-900 px-5 py-3 text-sm font-medium text-white"
            >
              Save Finance Structure
            </button>
          </div>
        </div>
      ) : null}
