  ...
  return (
    <div className="min-h-screen p-8">
      <div className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <h1 className="text-3xl font-bold">Parent Dashboard</h1>
          <p className="mt-2 text-slate-600">A focused view of your children’s school information.</p>
        </div>
      </div>
            ...
            <div className="flex items-center gap-4">
              <div className="h-16 w-16 overflow-hidden rounded-full bg-slate-100">
                {data.parent.profilePhoto ? (
                  <img src={data.parent.profilePhoto} alt="Parent profile" className="h-full w-full object-cover" />
                ) : (
                  <div className="flex h-full items-center justify-center text-slate-400">No photo</div>
                )}
              </div>
              <div>
                <p className="text-lg font-semibold">{data.parent.firstName} {data.parent.lastName}</p>
                <p className="text-sm text-slate-500">Parent account</p>
              </div>
            </div>
            <div className="grid gap-3 text-sm text-slate-700">
              <div><span className="font-semibold">Email:</span> {data.parent.email ?? 'Not provided'}</div>
              <div><span className="font-semibold">Phone:</span> {data.parent.phone ?? 'Not provided'}</div>
              <div><span className="font-semibold">Address:</span> {data.parent.address ?? 'Not provided'}</div>
            </div>
            <form onSubmit={handleProfileSave} className="space-y-4">
              <div className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-600">
                <p className="font-semibold">Account settings</p>
                <p className="mt-2">Edit phone, address, occupation, and profile photo. Names and email are read-only.</p>
              </div>
              ...
              <div className="grid gap-3">
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-600">Profile photo URL</label>
                  <input
                    value={profileForm.profilePhoto}
                    onChange={(event) => setProfileForm((current) => ({ ...current, profilePhoto: event.target.value }))}
                    className="mt-2 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3"
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-5 space-y-3">
            {data.children?.length === 0 ? (
              <p className="text-sm text-slate-500">No children are currently linked to this account.</p>
            ) : (
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {data.children?.map((child) => (
                    ...
                    >
                      ...
                      <p className="text-sm text-slate-500">{child.admissionNumber}</p>
                      <p className="text-sm text-slate-500">{child.className ?? 'No class'} • {child.academicYear ?? 'No year'}</p>
                    </button>
                  ))}
                </div>
              </div>
        ...
        <div className="mt-6 grid gap-6 xl:grid-cols-[1.4fr_1fr]">
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                <div className="rounded-3xl bg-slate-50 p-4">
                  ...
                  <p className="mt-2 text-lg font-semibold">{data.student.profile.firstName} {data.student.profile.lastName}</p>
                </div>
                <div className="rounded-3xl bg-slate-50 p-4">
                  <p className="text-sm text-slate-500">Admission</p>
                  <p className="mt-2 text-lg font-semibold">{data.student.profile.admissionNumber}</p>
                </div>
                <div className="rounded-3xl bg-slate-50 p-4">
                  <p className="text-sm text-slate-500">Class</p>
                  <p className="mt-2 text-lg font-semibold">{data.student.profile.className ?? 'Not assigned'}</p>
                </div>
                <div className="rounded-3xl bg-slate-50 p-4">
                  <p className="text-sm text-slate-500">Academic year</p>
                  <p className="mt-2 text-lg font-semibold">{data.student.profile.academicYear ?? 'Not assigned'}</p>
                </div>
              </div>
            </section>
                ...
                <table className="min-w-full text-left text-sm text-slate-700">
                  ...
                  <tbody>
                    {data.student.attendance.length === 0 ? (
                      <tr>
                        <td colSpan={4} className="py-4 text-sm text-slate-500">No attendance records available.</td>
                      </tr>
                    ) : (
          ...
          <section className="space-y-6">
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="mt-5 space-y-4 text-sm text-slate-700">
                <div className="rounded-3xl bg-slate-50 p-4">
                  ...
                  <p className="mt-2 text-lg font-semibold">UGX {data.student.finance.totalPaid.toLocaleString()}</p>
                </div>
                <div className="rounded-3xl bg-slate-50 p-4">
                  <p className="text-slate-500">Term fees</p>
                  <p className="mt-2 text-lg font-semibold">UGX {data.student.finance.payments.reduce((sum, payment) => sum + payment.amount, 0).toLocaleString()}</p>
                </div>
                <div className="rounded-3xl bg-slate-50 p-4">
                  <p className="text-slate-500">Balance</p>
                  <p className="mt-2 text-lg font-semibold">UGX {Math.max(0, data.student.finance.payments.reduce((sum, payment) => sum + payment.amount, 0) - data.student.finance.totalPaid).toLocaleString()}</p>
                </div>
              </div>
            </section>
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <p className="mt-4 text-sm text-slate-500">This section is prepared for future expansion and will show subject results, exam scores, and teacher comments.</p>
            </section>
          </section>
        </div>
      ) : (
        <div className="mt-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <p className="text-sm text-slate-500">Select a child to see their attendance and finance details.</p>
        </div>
      )}
    </div>
  );
